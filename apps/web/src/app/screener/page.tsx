...
export default function ScreenerPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fadeIn">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">AI 智能選股 (Beta)</h1>
        <p className="text-gray-500">
          輸入任何投資條件（如「高殖利率」、「AI 伺服器供應鏈」），讓 AI 為您生成清單並即時驗證市場數據。
        </p>
      </div>
                ...
                {stocks.length > 0 ? (
                     ...
                     return (
                        <tr key={stock.symbol} className="hover:bg-gray-50 transition-colors">
                          <td className="px-6 py-4 whitespace-nowrap">
                            <Link href={`/stocks/${stock.symbol}`} className="flex items-center">
                          ...
                          <td className={`px-6 py-4 whitespace-nowrap text-right text-sm font-medium ${isPositive ? 'text-green-600' : 'text-red-600'}`}>
                            {isPositive ? '▲' : '▼'} {Math.abs(stock.change)} ({Math.abs(stock.changePercent * 100).toFixed(2)}%)
                          </td>
                  ...
                  <tr>
                    <td colSpan={5} className="px-6 py-12 text-center text-gray-500">
                       ...
                       <p className="text-lg font-medium text-gray-900">找不到符合條件的股票</p>
                       <p className="text-sm mt-1">AI 已生成候選清單，但經過即時報價與條件過濾後，沒有股票留存。</p>
                       <p className="text-sm mt-1">請嘗試放寬價格範圍或使用更通用的關鍵字。</p>
                    </td>
                  </tr>
                )}
              </tbody>
