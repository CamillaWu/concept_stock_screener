...
export default function HomePage() {
  ...
  const handleSearch = async (query: string) => {
    const url = `${process.env.NEXT_PUBLIC_API_URL}/api/search?q=${encodeURIComponent(query)}`;
    await execute();
  };
      ...
      {data && (
        <div className="card">
          ...
          {data.stocks.length > 0 && (
            <div className="mb-6">
              <h3 className="text-lg font-medium mb-3">股票 ({data.stocks.length})</h3>
                  ...
                  <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                    ...
                    <div className="text-right">
                      ...
                      <div className={`text-sm ${stock.change >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                        {stock.change >= 0 ? '+' : ''}{stock.change} ({stock.changePercent >= 0 ? '+' : ''}{(stock.changePercent * 100).toFixed(2)}%)
                      </div>
                    </div>
                  </div>
                ))}
          ...
          {data.concepts.length > 0 && (
            <div>
              <h3 className="text-lg font-medium mb-3">概念股 ({data.concepts.length})</h3>
              <div className="space-y-2">
                {data.concepts.map((concept: any, index: number) => (
                  <div key={index} className="p-3 bg-gray-50 rounded-lg">
                    ...
                    <div className="flex flex-wrap gap-1 mt-2">
                      {concept.keywords.map((keyword: string, kIndex: number) => (
                        <span key={kIndex} className="px-2 py-1 bg-blue-100 text-blue-800 text-xs rounded-full">
                          {keyword}
                        </span>
                      ))}
                    </div>
